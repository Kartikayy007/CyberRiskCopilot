...
def compute_scores() -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        ...
        ransomware = intel_ransomware or kev_ransomware
        components = {
            ...
            "campaign_match": (1.0 if ransomware else 0.6 if intel_matches else 0.0),
            "business_impact": _business_impact_score(row),
            "aging": _aging_score(row.get("days_open")),
            "control_gap": min(
                (0.5 if str(row.get("edr_installed")).strip().lower() == "no" else 0.0)
                + (0.5 if str(row.get("patch_available")).strip().lower() == "no" else 0.0),
                1.0,
            ),
        }
        ...
        rows.append(
            {
                ...
                "ransomware_campaign_matched": ransomware,
